...
public class HashPercentageSet<T> implements PercentageSet<T>, Iterable<T> {
	...
	public T get() {
		...
		T result = null;
		double current = 0.0;
		Entry<T, Byte>[] entries = prepare(percentages.entrySet().toArray(new Entry[percentages.size()]));
		for (int i = 0; i < entries.length; i++) {
			Entry<T, Byte> entry = entries[i];
			result = entry.getKey();
			current += (double)entry.getValue() / 100;
			double next = 0.0;
			if (i + 1 < entries.length) next = (double) entries[i + 1].getValue() / 100;
			if (current >= random && next < current) break;
		}
		return result;
	}
	...
	private Entry<T, Byte>[] prepare(Entry<T, Integer>[] entries) {
		...
		for (int i = 0; i < entries.length; i++) preparedPercents[i] = Maps.immutableEntry(entries[i].getKey(), ((byte)(entries[i].getValue() / coefficient)));
		return preparedPercents;
	}
	
	private int summary(Entry<T, Integer>[] entries) {
		int summary = 0;
		for (Entry<?, Integer> pair : entries) summary += pair.getValue();
		return summary;
	}
}
